...
public class EngineUtil {
    ...
    private static void init() {
        ...
        NativeLong ret = INSTANCE.ASFInitEngine(FaceLibrary.ASF_DETECT_MODE_IMAGE, OrientPriority.AFD_FSDK_OPF_0_ONLY, 16, 50, mask, phEngine);
    ...
    public static MultiFaceInfo detectFaces(BufferedImage image) {
        ...
        getInstance().ASFDetectFaces(phEngine.getValue(), image.getWidth(), image.getHeight(), ColorFormat.ASVL_PAF_RGB24_B8G8R8, bufferInfo.buffer, detectFaces);
    ...
    public static FaceFeature extractFeature(SingleFaceInfo faceInfo, BufferedImage image) {
        ...
        image = convertImageTo4Times(image);
        BufferInfo bufferInfo = ImageLoader.getBGRFromFile(image);
        getInstance().ASFFaceFeatureExtract(phEngine.getValue(), image.getWidth(), image.getHeight(), ColorFormat.ASVL_PAF_RGB24_B8G8R8, bufferInfo.buffer, faceInfo, feature);
