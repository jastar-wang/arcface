...
public interface FaceLibrary extends Library {
    ...
    /**
     * @param detectMode [in] VIDEO 模式/IMAGE 模式 VIDEO 模式:处理连续帧的图像数据，并返回检测结果，需要将所有图
     * 像帧的数据都传入接口进行处理; IMAGE 模式:处理单帧的图像数据，并返回检测结果
     * @param detectFaceOrientPriority [in] 检测脸部的角度优先值，推荐仅检测单一角度，效果更优
     * @param detectFaceScaleVal [in]
     * 用于数值化表示的最小人脸尺寸，该尺寸代表人脸尺寸相对于图片长 边的占比。 video 模式有效值范围[2,16], Image
     * 模式有效值范围[2,32] 推荐值为 16
     * @param detectFaceMaxNum [in] 最大需要检测的人脸个数[1-50]
     * @param combinedMask [in] 用户选择需要检测的功能组合，可单个或多个
     * @param phEngine [out] 初始化返回的引擎 handle
     * @return
     */
    NativeLong ASFInitEngine(int detectMode, int detectFaceOrientPriority, int detectFaceScaleVal,
                             int detectFaceMaxNum, int combinedMask, PointerByReference phEngine);

    /**
     * @param hEngine [in] 引擎 handle
     * @param width [in] 图片宽度为 4 的倍数且大于 0
     * @param height [in] YUYV/I420/NV21/NV12 格式的图片高度为 2 的倍数，BGR24 格式的图片高度不限制
     * @param format [in] 颜色空间格式
     * @param imgData [in] 图片数据
     * @param detectedFaces [out] 检测到的人脸信息
     * @return
     */
     ...
     * @param format [in] 颜色空间格式
     * @param imgData [in] 图片数据
     * @param faceInfo [in] 单张人脸位置和角度信息
     * @param feature [out] 人脸特征
     * @return
     */
